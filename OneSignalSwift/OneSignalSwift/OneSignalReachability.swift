...
class OneSignalReachability : NSObject {
    ...
    var reachabiliyRef : SCNetworkReachabilityRef!
     ...
     */
    class func reachabilityForInternetConnection() -> OneSignalReachability? {
        ...
        let _ = withUnsafePointer(&zeroAddress) { return self.reachabilityWithAddress(UnsafePointer($0))}
    ...
    class func reachabilityWithAddress(hostAddress : UnsafePointer<sockaddr>) -> OneSignalReachability? {
        
        if let reachability = SCNetworkReachabilityCreateWithAddress(kCFAllocatorDefault, hostAddress) {
            let returnValue = OneSignalReachability()
            returnValue.reachabiliyRef = reachability
            returnValue.alwaysReturnLocalWiFiStatus = false
